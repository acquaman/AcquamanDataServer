...
class AMDSErrorReport {

public:
	enum Level {Information, Alert, Serious, Debug} ;

	virtual ~AMDSErrorReport();
	AMDSErrorReport(const QObject* src=0, Level l=Alert, int code=0, const QString& desc = "") : source(src), level(l), errorCode(code), description(desc) {}
...
Q_DECLARE_METATYPE(AMDSErrorReport)
...
*/
class AMDSErrorMon : public QObject {
...
public:
	...
	void exteriorReport(const AMDSErrorReport &e){
		reportF(e);
	}

	/// Report an error. This function is thread-safe.
	static void report(const AMDSErrorReport& e, bool showTimeoutMsgBox=false, QString msgBoxTitle="Information") {
	...
	static void report(const QObject *src = 0, AMDSErrorReport::Level level = AMDSErrorReport::Alert, int code = 0, const QString &desc = "", bool showTimeoutMsgBox=false, QString msgBoxTitle="Information")
	{
		mon()->reportF(AMDSErrorReport(src, level, code, desc), showTimeoutMsgBox, msgBoxTitle);
	}
	...
	{
		mon()->reportF(AMDSErrorReport(src, AMDSErrorReport::Information, code, desc), showTimeoutMsgBox, msgBoxTitle);
	}
	...
	{
		mon()->reportF(AMDSErrorReport(src, AMDSErrorReport::Alert, code, desc), showTimeoutMsgBox, msgBoxTitle);
	}
	...
	{
		mon()->reportF(AMDSErrorReport(src, AMDSErrorReport::Serious, code, desc), showTimeoutMsgBox, msgBoxTitle);
	}
	...
	{
		mon()->reportF(AMDSErrorReport(src, AMDSErrorReport::Debug, code, desc), showTimeoutMsgBox, msgBoxTitle);
	}
...
protected:
	...
	void reportF(AMDSErrorReport e, bool showTimeoutMsgBox=false, QString msgBoxTitle="Alert");
...
protected slots:
	...
	void reportI(AMDSErrorReport e);

private:
	/// Singleton class.  Private Constructor:
	virtual ~AMDSErrorMon();
	explicit AMDSErrorMon();

	/// class-wide instance variable
	static AMDSErrorMon* instance_;
